# make_synthetic_data_TL.py
import os
import numpy as np
import pandas as pd
import torch

from recovery_tests.grid_TL.TL_with_inferred_rho.simulator_TL import sample
...
# Output
OUT_CSV = "synthetic_data/synthetic_data_TL.csv"
...
def build_params_per_trajectory(
    ...
    """
    Returns params of shape (5, N): [alpha, mu, k, d, rho] per trajectory.
    Draws k ~ kappa_pool with replacement.
    """
    rng = np.random.default_rng(seed)
    k_draws = rng.choice(kappa_pool, size=N, replace=True)

    params = torch.empty((5, N), device=device, dtype=torch.float32)
...
def simulate_and_dilute(params_5xN: torch.Tensor, T_hours: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Runs forward model and returns plated counts (c1, c2, c3) as int tensors on CPU.
    """
    N = T_hours.numel()
    _, E = sample(params_5xN, T=T_hours, N=N, device=params_5xN.device)
...
def main():
    ...
    params_5xN = build_params_per_trajectory(
        alpha=ALPHA, mu=MU, d=D, rho=RHO,
        kappa_pool=kappa_pool, N=N, seed=SEED, device=DEVICE
    )

    # Forward simulate + dilute
    c1, c2, c3 = simulate_and_dilute(params_5xN, T_hours)
