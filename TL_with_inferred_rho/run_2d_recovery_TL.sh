...
export SCRATCH_RUN_DIR=/scratch/cylu1/recovery_2d_TL_${SLURM_JOB_ID}
...
OUTDIR_REL="recovery_tests/grid"

# === Run Script ===
python run_2d_recovery_TL.py \
  --data "$DATA_PATH" \
  --kappa "$KAPPA_NPZ" \
  --outdir "$OUTDIR_REL" \
  --device cuda:0 \
...
OUTDIR=~/recovery_2d_runs_TL/${SLURM_JOB_ID}
mkdir -p "$OUTDIR"
