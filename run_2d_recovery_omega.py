...
MU_TRUE = 0.5
D_TRUE = 0.12
OMEGA_TRUE = D_TRUE / MU_TRUE

# Grids
ALPHAS = np.linspace(0.0, 0.1, 11)[1:]  # 10
MUS    = np.linspace(0.2, 1.0, 11)[1:]  # 10
OMEGAS = np.linspace(0.0, 0.9, 11)[1:]  # enforce model assumption d < mu (i.e., omega < 1)
...
def run_2d_sweep(
    ...
    truth=(ALPHA_TRUE, MU_TRUE, OMEGA_TRUE),
    ...
    Output CSV columns: idx, alpha, mu, omega, d, loglike
    """
    out_path = os.path.join(outdir, f"{name}.csv")
    header = ["idx", "alpha", "mu", "omega", "d", "loglike"]
    ...
    with open(out_path, "a", newline="") as f:
        ...
        for x in g1:
            for y in g2:
                ...
                elif w1 == "omega": o = float(x)
                else: raise ValueError(f"bad w1={w1}")

                if w2 == "alpha": a = float(y)
                elif w2 == "mu":  m = float(y)
                elif w2 == "omega": o = float(y)
                else: raise ValueError(f"bad w2={w2}")

                # enforce model assumption d < mu (i.e., omega < 1)
                if o >= 1.0:
                    ll = -np.inf
                else:
                    ll = loglike_alpha_mu_omega(dataset, a, m, o)

                d = o * m
                w.writerow([idx, a, m, o, d, ll])
                wrote += 1
                idx += 1
...
def main():
    ...
    truth = (ALPHA_TRUE, MU_TRUE, OMEGA_TRUE)
    print(f"Truth: alpha={truth[0]}, mu={truth[1]}, omega={truth[2]}")
    ...
    run_2d_sweep(dataset, "alpha_omega", "alpha", ALPHAS, "omega", OMEGAS,
                truth=truth, outdir=args.outdir, flush_every=args.flush_every)

    run_2d_sweep(dataset, "mu_omega", "mu", MUS, "omega", OMEGAS,
                truth=truth, outdir=args.outdir, flush_every=args.flush_every)
