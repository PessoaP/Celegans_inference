...
try:
    for s in tqdm(range(start_step, total_steps)):
        ...
        any_moved = False   # Track if any coordinate was accepted

        for i in infer_idx:  # infer_idx set in the initialization block of the code!
            
            # Propose and possibly update parameters
            params_new, lp_new, state, accepted = mcmc.next_MCMC_sample(
                logposterior, params, lp, state,
                greedy=greedy, adapt=adapt, sample_history=sample_history, 
        ...
        # After all coordinates updated in this sweep:
        if any_moved:
            stalled_steps = 0
        else:
            stalled_steps += 1
